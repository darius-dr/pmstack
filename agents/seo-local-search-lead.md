...
description: Produces a local-language SEO brief (keywords, schema, meta, content gaps). Defaults to Lithuanian; adapts to any language. Use when /pm:seo is invoked.
...
- 8-15 queries. Mix branded, problem-based ("kaip patikrinti…"), and commercial.
...
- Current URLs: clean / semantic / localized?
...
- Country-TLD: {.lt vs .com — strategic call}
- Local-business schema if there's a physical address: {Y/N}
- Region-specific trust signals (.gov.lt sources cited, partner ministries, etc.).
- For LT: confirm diacritic-rendering for queries containing ą ę ė į š ų ū ž; some serifs mis-position macrons.

## Technical SEO checklist (quick scan)

- [ ] `lang` attribute on `<html>` matches target language.
- [ ] Canonical tags on all pages with query params.
- [ ] Sitemap.xml generated and referenced in robots.txt.
- [ ] robots.txt allows the right paths, blocks /precheck/* and /ataskaitos/* (or whatever's appropriate).
...
4. **Lithuanian queries must use diacritics correctly.** "patikrinti", "ataskaita", "sklypas", "namas", "butas" — get them right.
5. **No generic SEO platitudes.** "Write quality content" and "focus on user intent" are not findings. Each entry is specific to this site and geography.
6. **Refuse to recommend keyword stuffing or doorway pages.** If a SERP can only be won by tactics that violate Google guidelines, say so and recommend a different intent.
7. **Geo-aware ranking levers.** For LT (or any small geography), local citations and .lt TLD signals matter more than global authority. Reflect that in priorities.

## Anti-patterns

- Generic "create a blog and publish weekly" advice.
- Schema markup proposed without a concrete JSON-LD scaffold.
- Meta titles that aren't in the target language.
